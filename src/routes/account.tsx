        ...
        <div className="mx-auto max-w-5xl px-6">
          <div className="flex flex-col md:flex-row items-start gap-10">
            ...
            <aside className="w-full md:w-72 shrink-0 space-y-6">
              <div className="rounded-3xl border border-border bg-white p-6 shadow-[var(--shadow-soft)] text-center">
                <div className="mx-auto size-20 rounded-full bg-gradient-ocean text-white grid place-items-center text-3xl font-black shadow-[var(--shadow-float)] mb-4">
                  {user.name.charAt(0).toUpperCase()}
                </div>
                <h1 className="text-xl font-bold tracking-tight">{user.name}</h1>
                <p className="text-sm text-muted-foreground mt-1 mb-6 truncate">{user.email}</p>
                <div className="h-px bg-border w-full mb-6" />
                <button
                  onClick={() => { logout(); window.location.href = "/"; }}
                  className="w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold text-destructive bg-destructive/10 hover:bg-destructive/20 transition-colors"
                >
                      ...
                      <div className="flex flex-wrap items-center justify-between gap-4 bg-muted/30 p-5 border-b border-border">
                        <div className="flex gap-6">
                          <div>
                            ...
                            <p className="text-sm font-semibold mt-0.5">{new Date(order.placedAt).toLocaleDateString()}</p>
                          </div>
                          <div>
                            <p className="text-[10px] uppercase tracking-widest text-muted-foreground font-semibold">Total</p>
                            <p className="text-sm font-semibold mt-0.5">{formatUSD(order.total)}</p>
                          </div>
