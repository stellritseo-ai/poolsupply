  ...
  const handleUpdateSecurity = async (e: React.FormEvent) => {
    ...
    const res = await updateSuperAdmin({
      data: {
        ...
        newUsername: secNewUsername || undefined,
    ...
    if (res.success) {
      ...
      if (secNewUsername) setCurrentUsername(secNewUsername);
            ...
            <div className="bg-white border border-slate-200/60 rounded-[2rem] p-6 shadow-sm relative overflow-hidden">
              ...
              <form onSubmit={handleUpdateSecurity} className="space-y-4">
                <label className="block">
                  ...
                  <input
                    type="password" required value={secCurrentPassword} onChange={(e) => setSecCurrentPassword(e.target.value)}
                    placeholder="Enter current password to authorize changes"
                    className="w-full h-11 px-3 rounded-xl border border-slate-200 bg-slate-50 text-xs focus:outline-none focus:border-primary transition"
                  />
                </label>
                
                <div className="pt-4 border-t border-slate-100">
                  <label className="block mb-4">
                    <span className="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1.5">New Username (Optional)</span>
                    <input
                      type="text" value={secNewUsername} onChange={(e) => setSecNewUsername(e.target.value)}
                      placeholder="Leave blank to keep current username"
                      className="w-full h-11 px-3 rounded-xl border border-slate-200 bg-slate-50 text-xs focus:outline-none focus:border-primary transition"
                    />
                  </label>
                  <label className="block">
                    <span className="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1.5">New Password (Optional)</span>
                    <input
