    ...
    const itemListLd = {
      ...
      itemListElement: (loaderData?.initialProducts || []).map((prod: any, index: number) => ({
        ...
        item: {
          "@type": "Product",
          name: prod.name,
          url: `https://poolsupplywholesalers.com/products/${prod.id}`,
          image: prod.img,
...
function getCategoryName(slug: string): string {
  switch (slug.toLowerCase()) {
    ...
    case "safety-accessibility":
      return "Safety & Accessibility";
    case "pool-pumps":
    case "pumps":
      return "Pool Pumps";
    case "pool-lights":
    ...
    case "filters":
      return "Pool Filters";
    case "electric-heat-pumps":
      return "Electric Heat Pumps";
    case "automation-systems":
    case "automation":
      return "Automation Systems";
    case "blowers":
      return "Blowers";
    case "chlorine-feeders":
      return "Chlorine Feeders";
    case "salt-systems":
      return "Salt Systems";
    case "skid-systems":
      return "Skid Systems";
...
function getCategoryContent(slug: string) {
  const content = {
    "pool-pumps": {
      ...
      faqs: [
        {
          q: "Do I need a variable speed pool pump?",
          a: "Yes, in most cases. Department of Energy (DOE) regulations mandate that most new pool pumps must be variable speed to meet minimum efficiency standards. They also save up to 80% on energy costs compared to single speed models.",
        },
        {
          q: "How do I size a pool pump?",
          a: "Sizing depends on your pool's volume and the required turnover rate. Generally, you want a pump that can circulate all the water in your pool in 6-8 hours. A pump that is too large will waste energy and potentially damage your filter.",
        },
      ],
    },
    "pool-heaters": {
      ...
      intro:
        "Extend your swimming season with high-performance pool heating solutions. Our selection includes gas heaters (natural gas and propane), energy-efficient electric heat pumps, and replacement heating elements. Sourced from industry leaders like Raypak, Pentair MasterTemp, and Jandy JXi, these heaters offer rapid heat-up times and superior energy efficiency for both residential and commercial applications.",
      subcategories: [
        "Natural Gas Heaters",
        "Propane Heaters",
        "Electric Heat Pumps",
      ...
      faqs: [
        {
          ...
          a: "Heater size is measured in BTUs. As a general rule, you need 100,000 BTUs per 10,000 gallons of pool water, but this varies based on your climate, whether you use a pool cover, and how quickly you want the water to heat.",
        },
        {
          q: "Gas heater vs. Heat pump: Which is better?",
          a: "Gas heaters heat water quickly and work regardless of air temperature, making them ideal for quick heating and colder climates. Heat pumps are more energy-efficient and cost less to operate, but they rely on ambient heat and work best when air temperatures are above 50°F.",
        },
      ],
    },
    "pool-filters": {
      ...
      intro:
        "Maintain crystal clear water with our professional-grade pool filtration systems. We offer a full range of cartridge filters, sand filters, and D.E. (Diatomaceous Earth) filters for any pool size. Choose from trusted models like the Pentair Clean & Clear, Hayward SwimClear, and Jandy DEV series for superior particulate removal and extended cleaning cycles.",
      subcategories: ["Cartridge Filters", "Sand Filters", "D.E. Filters", "Filter Elements"],
      faqs: [
        {
          q: "Which type of pool filter is best?",
          a: "D.E. filters offer the finest filtration (down to 2-5 microns) but require the most maintenance. Cartridge filters provide excellent filtration (10-20 microns) and are easy to maintain by hosing off. Sand filters are the easiest to maintain (just backwash) but offer the least fine filtration (20-40 microns).",
        },
        {
          q: "How often should I clean my pool filter?",
          a: "Clean your filter when the pressure gauge reads 8-10 PSI above the clean starting pressure. For cartridge filters, this is typically every 3-6 months; for sand and D.E. filters, backwashing is usually required every 1-3 months depending on usage.",
        },
      ],
    },
    "automation-systems": {
      ...
      intro:
        "Take complete control of your pool and spa equipment from anywhere with advanced automation systems. Our smart controllers allow you to manage pumps, heaters, lighting, and water features from your smartphone. Upgrade to Pentair IntelliCenter, Hayward OmniLogic, or Jandy AquaLink systems for the ultimate in convenience, energy savings, and modern pool management.",
      subcategories: ["Smart Controllers", "Valve Actuators", "Wireless Remotes", "Control Boards"],
      faqs: [
        {
          q: "Can I upgrade my existing pool to an automation system?",
          a: "Yes. Most existing pools can be retrofitted with an automation system. The main requirement is ensuring your new controller is compatible with your existing variable speed pump and heater.",
        },
        {
          q: "Do pool automation systems save money?",
          a: "Absolutely. Automation allows you to run equipment only when needed, optimize pump speeds for different tasks, and prevent heaters from running unnecessarily, significantly reducing energy consumption.",
        },
      ],
    },
    "salt-systems": {
      ...
      intro:
        "Enjoy softer, clearer water without the harsh smell of traditional liquid chlorine. Our wholesale salt chlorine generators automatically convert dissolved salt into pure chlorine, maintaining perfect sanitization levels. We carry complete systems and replacement salt cells for popular brands like Pentair IntelliChlor, Hayward AquaRite, and AutoPilot.",
      subcategories: [
        "Complete Salt Systems",
      ...
      faqs: [
        {
          ...
          a: "A typical salt cell lasts 3-7 years (or roughly 10,000 hours of operation) depending on usage, water chemistry balance, and how often it is cleaned.",
        },
        {
          q: "Do salt pools still use chlorine?",
          a: "Yes. A salt system is actually a small chlorine factory on your equipment pad. It uses electrolysis to separate the chlorine from the salt molecule (NaCl), providing a steady stream of pure chlorine to the pool.",
        },
      ],
    },
    "pool-cleaners": {
      ...
      faqs: [
        {
          ...
          a: "Yes. Robotic cleaners are the most energy-efficient option as they don't rely on your pool pump. They also have their own filtration system, which reduces the load on your main pool filter.",
        },
        {
          q: "What's the difference between suction and pressure cleaners?",
          a: "Suction cleaners connect to your skimmer or dedicated suction line and use the pump's suction to move and vacuum debris into your pool filter. Pressure cleaners connect to the return line (often requiring a booster pump) and use water pressure to push debris into an attached bag, saving your main filter from filling up.",
        },
      ],
    },
  };
...
function CategoryPage() {
  ...
  const [sortBy, setSortBy] = useState<"rating-desc" | "price-asc" | "price-desc" | "name-asc">(
    "rating-desc",
  );
      ...
      <main className="flex-1 pt-28 pb-20">
          ...
          <div className="mx-auto max-w-7xl px-4 sm:px-6 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            <div>
              ...
              <p className="mt-2 text-xs sm:text-sm text-muted-foreground max-w-xl leading-relaxed font-medium">
                Premium commercial-grade {categoryName.toLowerCase()} engineered for high
                reliability, performance, and efficiency. Enjoy exclusive bulk wholesale rates.
              </p>
            </div>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          ...
          <div className="grid lg:grid-cols-[260px_1fr] gap-8 items-start">
            {/* Sidebar Filters */}
            <aside
              className={`space-y-6 lg:sticky lg:top-28 p-5 lg:p-0 rounded-2xl lg:rounded-none bg-white lg:bg-transparent border lg:border-none border-slate-200 shadow-sm lg:shadow-none ${mobileFiltersOpen ? "block mb-6 lg:mb-0" : "hidden lg:block"}`}
            >
              ...
              {isLoading ? (
                <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-3 sm:gap-6">
                  {Array.from({ length: 10 }).map((_, i) => (
                    <div
                      key={i}
                      className="rounded-2xl sm:rounded-3xl bg-slate-100 animate-pulse"
                      style={{ height: 320 }}
                    />
                  ))}
                </div>
                ...
                <div
                  className={`grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-3 sm:gap-6 transition-opacity duration-200 ${isPlaceholderData ? "opacity-60" : "opacity-100"}`}
                >
                  {products.map((p, i) => (
                ...
                <div className="text-center py-20 bg-surface rounded-3xl border border-dashed border-border p-6">
                  ...
                  <p className="text-xs text-muted-foreground/75 mt-1">
                    Try clearing selected filters or check another category.
                  </p>
                  <button
                    onClick={() => {
                      setSearchQuery("");
