...
export const getDatabaseStats = createServerFn({ method: "POST" }).handler(async () => {
  try {
    ...
    const [
      ...
      db
        ...
        .catch(() => 0),
      db
        .collection("admin_security_locks")
        .countDocuments()
        .catch(() => 0),
      db
        .collection("settings")
      ...
      if (products.length > 0) {
        ...
        for (const item of products) {
          const existing = await productsCol.findOne({
            $or: [{ sku: item.sku }, { id: item.id }, { _id: item.id as any }],
          });
          if (existing) {
            await productsCol.replaceOne({ _id: existing._id }, { ...item, _id: existing._id });
          } else {
