...
export function BestSellers() {
  ...
  const allPoolProducts = useMemo(() => {
    ...
    const normalizeWithSalePrice = (p: any): Product => {
      ...
      const rawSale = p.salePrice != null && Number(p.salePrice) > 0
        ? Number(p.salePrice)
        : (p.msrp && Number(p.msrp) > rawPrice ? Number(p.msrp) : Math.round(rawPrice * 1.25 * 100) / 100);

      const higherMsrp = p.msrp && Number(p.msrp) > rawSale
        ? Number(p.msrp)
        : Math.round(rawSale * 1.2 * 100) / 100;
  ...
  const autoCleaners = useMemo(() => {
    return allPoolProducts.filter((p) => {
      ...
      const isAuto = /cleaner|robotic|robo|polaris|dolphin|trivac|aquavac|tigershark|sharkvac|baracuda|prowler/i.test(pName);
      const isBlacklisted = BLACKLIST_NAMES.some((b) => pName.includes(b));
      const notSmallPart = !/cable|bag|tire|brush|hose|bracket|adapter|fitting|part|applicator/i.test(pName) && p.price > 300;
  ...
  const pumps = useMemo(() => {
    return allPoolProducts.filter((p) => {
      ...
      const isBlacklisted = BLACKLIST_NAMES.some((b) => pName.includes(b));
      const notSmallPart = !/seal|gasket|impeller|lid|basket|fitting|union|adhesive|flange|switch/i.test(pName) && p.price > 400;
  ...
  const heaters = useMemo(() => {
    return allPoolProducts.filter((p) => {
      ...
      const isRealHeater = /heater|heating|heat pump|lonx|jxi|dual fuel|e3t|18kw|btu/i.test(pName);
      const isBlacklisted = BLACKLIST_NAMES.some((b) => pName.includes(b));
      const notSmallPart = !/board|kit|switch|sensor|wire|governer|limit|fitting|union|105k lp/i.test(pName) && p.price > 400;
  ...
  const lights = useMemo(() => {
    return allPoolProducts.filter((p) => {
      ...
      const isBlacklisted = BLACKLIST_NAMES.some((b) => pName.includes(b));
      const notSmallPart = !/cable|screw|gasket|lens|ring|adapter kit|housing|cord only/i.test(pName) && p.price > 200;
        ...
        <div className="flex items-end justify-between flex-wrap gap-4 mb-6 sm:mb-8">
          <div>
            ...
            <p className="mt-2 text-xs sm:text-sm text-muted-foreground max-w-xl">
              Automatic robotic pool cleaners, commercial-grade pumps, high-efficiency heaters &amp; LED lighting at direct trade sale prices.
            </p>
          </div>
          ...
          >
            Shop All Cleaners <ArrowRight className="size-3.5 group-hover:translate-x-1 transition-transform" />
          </Link>
        </div>
          ...
          {tabs.map((tab) => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-1.5 sm:gap-2 px-3.5 sm:px-4 py-2 rounded-full text-xs font-bold whitespace-nowrap transition-all ${isActive
              ...
              >
                ...
                <span>{tab.label}</span>
                {tab.badge && (
                  <span className={`text-[9px] font-black uppercase px-1.5 py-0.2 rounded-full ${isActive ? "bg-cyan-400 text-slate-900" : "bg-cyan-50 text-cyan-700 border border-cyan-200"
                    }`}>
                    {tab.badge}
                  </span>
                )}
